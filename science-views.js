...
function toggleElementLearned(symbol, tier) {
  ...
  if (idx >= 0) {
    ...
    if (s.elementsLearned.length === 1) {
      awardXP(5, "Learned your first element!", "sci-first-element");
    }
  }
  checkScienceBadges();
  saveState();
...
function nextScienceQuizQuestion() {
  ...
  if (qs.current >= qs.questions.length) {
    ...
    awardXP(xp, `Element Explorer (${qs.tier}) \u2014 ${qs.score}/10`, `sci-explorer-${qs.tier}`);
    checkScienceBadges();
    saveState();
    renderElementQuiz(document.getElementById("main-content"), qs.tier);
...
function nextSorterItem() {
  ...
  if (gs.current >= gs.items.length) {
    ...
    awardXP(xp, `State Sorter \u2014 ${gs.score}/10`, "sci-state-sorter");
    checkScienceBadges();
    saveState();
    renderStateSorter(document.getElementById("main-content"));
    return;
...
function doMix() {
  ...
  if (recipe) {
    ...
    if (!s.recipesDiscovered.includes(recipe.name)) {
      ...
      awardXP(15, `Discovered ${recipe.name} in Mix Lab!`, `sci-mixlab-${recipe.name.replace(/ /g, "-").toLowerCase()}`);
      checkScienceBadges();
      saveState();
    }
  }
...
function checkEquation() {
  ...
  if (correct) {
    ...
    if (!s.compoundsDiscovered.includes(compound.name)) {
      s.compoundsDiscovered.push(compound.name);
      awardXP(20, `Built ${compound.name} in Equation Builder!`, `sci-equation-${compound.name.replace(/ /g, "-").toLowerCase()}`);
      checkScienceBadges();
      saveState();
    }
  } else {

...
function createDefaultUser(name) {
  return {
    ...
    bestVoiceAccuracy: 0,
    voiceHistory: [],
    pet: null,  // LEGACY - migrated to pets[]
...
let _lastLoadedAt = null; // Track when we last loaded from DB

function _mergePets(localKids, serverKids) {
  // Merge pet collections: keep all pets from both local and server
  if (!serverKids) return;
  localKids.forEach(localKid => {
    const serverKid = serverKids.find(sk => sk.name === localKid.name);
    if (!serverKid || !serverKid.pets) return;
    if (!localKid.pets) localKid.pets = [];
    // Add any server pets we don't have locally
    serverKid.pets.forEach(sp => {
      if (!localKid.pets.some(lp => lp.typeId === sp.typeId)) {
        localKid.pets.push(sp);
      }
    });
  });
}
...
async function _doSave() {
  ...
  // Sync active pet back to pets array before saving
  state.kids.forEach(kid => {
    if (kid.pet && kid.pets && kid.pets.length > 0) {
      kid.pets[kid.activePetIndex || 0] = kid.pet;
    }
  });

  // Check for conflicts: if server data is newer, merge pets first
  try {
    const { data: serverRow } = await sb.from("progress").select("data, updated_at").eq("id", state.userId).single();
    if (serverRow && _lastLoadedAt && serverRow.updated_at > _lastLoadedAt) {
      // Server has newer data — merge pet collections so we don't lose pets
      console.log("Merge: server updated since our last load, merging pets...");
      const serverKids = serverRow.data ? serverRow.data.kids : null;
      _mergePets(state.kids, serverKids);
    }
  } catch (e) {
    console.warn("Conflict check failed, saving anyway:", e.message);
  }

  const now = new Date().toISOString();
  const payload = { kids: state.kids, activeKidIndex: state.activeKidIndex };
  const { error } = await sb.from("progress")
    .upsert({ id: state.userId, data: payload, updated_at: now });
  if (error) {
...
function loadKidsFromData(rawData) {
  ...
  state.kids.forEach(kid => {
    ...
    // Migrate single pet to pets array
    if (!kid.pets) kid.pets = [];
    if (kid.activePetIndex === undefined) kid.activePetIndex = 0;
    if (kid.pet && kid.pets.length === 0) {
      kid.pets.push(kid.pet);
      kid.activePetIndex = 0;
    }
    // Sync active pet pointer
    if (kid.pets.length > 0) {
      if (kid.activePetIndex >= kid.pets.length) kid.activePetIndex = 0;
      kid.pet = kid.pets[kid.activePetIndex];
    } else {
      kid.pet = null;
    }
...
function switchKid(index) {
  ...
  // Sync pet back to pets array before switching
  if (state.user && state.user.pet && state.user.pets && state.user.pets.length > 0) {
    state.user.pets[state.user.activePetIndex || 0] = state.user.pet;
  }
...
function renderPet(container) {
  if (!state.user.pets || state.user.pets.length === 0) {
    renderPetAdoption(container);
    return;
  }
  // Ensure pet pointer is synced
  state.user.pet = state.user.pets[state.user.activePetIndex || 0];
            ...
            ${state.user.pets.map((p, i) => {
              ...
              return `
                <button class="pet-collection-card ${isActive ? 'active' : ''}" onclick="switchActivePet(${i})">
                  ...
                  ${isActive ? '<div class="pet-coll-active">★ Active</div>' : ''}
                </button>
              `;
            }).join('')}
            ${state.user.pets.length < PET_TYPES.length ? `
              <button class="pet-collection-card pet-coll-add" onclick="state.currentView='pet-adopt'; render();">
                <div class="pet-coll-emoji" style="font-size:2rem;opacity:0.5">+</div>
                <div class="pet-coll-name" style="color:var(--color-text-muted)">Adopt New</div>
              </button>
            ` : ''}
          </div>
        </div>
...
function renderPetAdoption(container) {
  container.innerHTML = `
    ...
    <div class="pet-adoption-grid">
      ${PET_TYPES.map(pet => {
        const owned = state.user.pets && state.user.pets.some(p => p.typeId === pet.id);
        const canAfford = (state.user.xp >= pet.cost || pet.cost === 0) && !owned;
        return `
          <div class="pet-adopt-card ${owned ? 'owned' : !canAfford ? 'locked' : ''}" onclick="${!owned && canAfford ? `adoptPet('${pet.id}')` : ''}">
    ...
    ${state.user.pets && state.user.pets.length > 0 ? '<div style="text-align:center;margin-top:var(--space-4)"><button class="welcome-btn" style="width:auto;padding:var(--space-2) var(--space-6)" onclick="navigate(\'pet\')">Back to My Pets</button></div>' : ''}
    <footer class="app-footer">
      <a href="https://www.perplexity.ai/computer" target="_blank" rel="noopener noreferrer">Created with Perplexity Computer</a>
    </footer>
  `;
...
function adoptPet(typeId) {
  ...
  if (state.user.pets.some(p => p.typeId === typeId)) {
    showToast("🐾", "Already Owned", `You already have a ${petType.name}!`);
    return;
  }
  ...
  state.user.pets.push(newPet);
  state.user.activePetIndex = state.user.pets.length - 1;
  state.user.pet = newPet;
...
function switchActivePet(index) {
  if (!state.user.pets || index < 0 || index >= state.user.pets.length) return;
  state.user.activePetIndex = index;
  state.user.pet = state.user.pets[index];
  saveStateNow();
  render();
  const petType = PET_TYPES.find(p => p.id === state.user.pet.typeId);
  showToast("🐾", "Switched!", `Now playing with ${state.user.pet.name} the ${petType ? petType.name : "Pet"}!`);
}
...
function givePetXP(amount) {
  if (!state.user.pets || state.user.pets.length === 0) return;
  // Active pet gets full XP share, others get a small passive share
  state.user.pets.forEach((p, i) => {
    const isActive = (i === state.user.activePetIndex);
    const xpGain = Math.ceil(amount * (isActive ? 0.5 : 0.1));
    p.xp += xpGain;
    if (isActive) {
      p.happiness = Math.max(0, p.happiness - 1);
    }
  });
  // Check evolution for active pet
  const prevStage = getPetStageIndex();
  const newStage = (function() {
    const pet = state.user.pet;
    for (let i = PET_STAGES.length - 1; i >= 0; i--) {
      if (pet.xp >= PET_STAGES[i].xpNeeded && pet.feedings >= PET_STAGES[i].feedingsNeeded) return i;
    }
    return 0;
  })();
  if (newStage > prevStage) {
